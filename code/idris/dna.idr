...
--countBase (h::t) x  = list.foldr plus (map baseNat x h)::(countBase t) 
countBase l x = list.foldr plus 0 (map (baseNat x) l)
--so what it does it is takes the baseNat output of (base -> nat) and
  --does it to the entire lsit (l) to get a lsit of zeroes and ones,
    --which it adds
